...
The following shows a `LimitRange` object for all components: pod, container,
image, image stream, or PVC. You can configure limits for any or all of these
components in the same object. You create a different `LimitRange` object for
each project where you want to control resources.

.Sample `LimitRange` object for a container
