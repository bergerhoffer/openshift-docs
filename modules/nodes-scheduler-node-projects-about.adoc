...
The Pod Node Selector admission controller also allows you to create a list of labels that are permitted in a specific project. This list lets developers know what labels are acceptable to use in a project and gives administrators greater control over labeling in a cluster.
...
. Evaluate the pod’s node selector against the namespace-specific list defined the plugin configuration file. Conflicts result in rejection.
