...
specified in the `LimitRange` object are assigned to the container.
...
than or equal to the `openshift.io/image-tags` constraint in the `LimitRange`
object.

* The number of unique references to images in an `ImageStream` specification
must be less than or equal to the `openshift.io/images` constraint in the `LimitRange` object.

.Image stream `LimitRange` object definition
